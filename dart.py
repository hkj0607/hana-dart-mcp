...
# DART Taxonomy 로드
TAXONOMY_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dart_taxonomy.json")
DART_TAXONOMY = {}
if os.path.exists(TAXONOMY_PATH):
    with open(TAXONOMY_PATH, 'r', encoding='utf-8') as _f:
        DART_TAXONOMY = json.load(_f)

# Taxonomy 기반 ACODE 매핑
ACODE_MAP = DART_TAXONOMY.get("acode_mapping", {})
ACCOUNT_MAP = DART_TAXONOMY.get("account_mapping", {})
...
async def search_audit_report_financial(
    ...
    Args:
        ...
        start_date: 시작일 (YYYYMMDD 형식, 예: 20230101)
        end_date: 종료일 (YYYYMMDD 형식, 예: 20261231)
        ctx: MCP Context 객체
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            for item in data.get("list", []):
                ...
                if "감사보고서" not in rpt_nm:
                    continue
                if report_type == "감사보고서" and "연결" in rpt_nm:
                    continue
                if report_type == "연결감사보고서" and "연결" not in rpt_nm:
                    continue
                audit_reports.append(item)

            if not audit_reports:
                return f"'{matched_name}'의 감사보고서를 찾을 수 없습니다. (기간: {start_date}~{end_date})"
            ...
            for report in audit_reports:
                ...
                result += f"## {report_nm} (접수일: {rcept_dt})\n\n"
